...
do
    ...
    $BIN/slice_data.pl $TEST_PRE $TEST_END_W_BUF < ${DATA_DIR}/${TRAIN_FILE} > ${TRAIN_DIR}/test-data-${TEST_START}.dat
...
TRAIN_END_W_BUF=${START_YEAR}12
TRAIN_TAG=${START_YEAR}01

$BIN/slice_data.pl 197001 ${TRAIN_END_W_BUF} < $DATA_DIR/${TRAIN_FILE} > ${TRAIN_DIR}/train-data.dat

$BIN/classify_data.py --config=${CONFIG_FILE} --default_gpu=/gpu:${GPU} --model_dir=${CHKPTS_NAME}-${TRAIN_TAG} --print_start=197506 --print_end=${TRAIN_END} \
