...
TRAIN_FILE=row-norm-all-100M-k64.dat
CONFIG_FILE=config/logreg-iter.conf
CHKPTS_NAME=${TRAIN_DIR}/chkpts-${TRAIN_DIR}
...
do
	...
	$BIN/train_log_reg.py --config=${CONFIG_FILE} --default_gpu=/gpu:${GPU} \
	    --num_unrollings=${NUM_UNROLLINGS} --train_datafile=${TRAIN_FILE} \
	...
	$BIN/classify_data.py --config=${CONFIG_FILE} --default_gpu=/gpu:${GPU} \
	    --num_unrollings=${NUM_UNROLLINGS} --model_dir=${CHKPTS_NAME}-${TEST_START} \
	    --print_start=${TEST_START} --print_end=${TEST_END} \
            --data_dir=. --test_datafile=${TRAIN_DIR}/test-data-${TEST_START}.dat \
	    --output=${TRAIN_DIR}/preds-${TEST_START}.dat > ${TRAIN_DIR}/results-${TEST_START}.txt
